...
class Laser(object):
    ...
    def scanProbability(self, z, x, gridmap):
        ...
        q=1
        (xr, yr, thetar) = x
        thetar = self.angleWrap(thetar)

        # pulled from table 6.1 
        for k in range(0, np.shape(z)[0]):
            lAngle = self.angleWrap(self.Angles[k])
            zt = z[k]
            
            (ztk, _) = self.rayTracing(xr = xr, yr= yr, thetar = thetar, 
                                          lAngle = lAngle, gridmap = gridmap)
            
            if 0<=zt and zt<= self.zMax:
                # compute inverse CDF from zero to zmax
                # normalizer = self.normal.ppf(self.zMax)
                normalizer = 1/(self.normal.cdf(self.zMax-ztk) - 0.5)
                hit = normalizer * self.normal.pdf(zt - ztk)
                rand = 1/ self.zMax
            else:
                hit = 0
                rand = 0

            # exponential pdf and inverse cdf
            if 0<=zt and zt<=ztk:
                short = (1/(1-self.exponential.pdf(ztk)))*self.lambdaShort*self.exponential.pdf(zt)
            else:
                short = 0
            ...
            # need to get zhit phit zshort zmax prand pshort pmax
            p = hit*self.pHit + short*self.pShort + mmax * self.pMax + rand*self.pRand
            q = q * p
        return q
